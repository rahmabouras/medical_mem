@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'diagnostic', 'title' => __('disgnostic')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            ...
                            <p class="card-category"> Les Diagnostic </p>
                        </div>
                                    ...
                                    @foreach ($diag as $row )
                                        <tr>
                                         ...
                                         . $row->listConsultation->DossierPatient->prenom_patient}} </td>
                                            <td>{{$row->date_diagnostic}}</td>
                                            <td>{{$row->resultat_diagnostic}}</td>

                                            <td class="td-actions text-right">
                                                <a rel="tooltip" class="btn btn-success btn-link"
                                                   href="{{route('diagnostic.edit',$row->id)}}"
                                                   data-original-title="" title="">
                                                    <i class="material-icons">edit</i>
                                                    <div class="ripple-container"></div>
                                                </a>
                                            <!-- <form action="{{route('diagnostic.destroy',$row->id)}}" method="post">
                                          @method('DELETE')
                                            @csrf
                                                ...
                                                <a data-type="delete" rel="tooltip" class="btn btn-success btn-link"
                                                   ...
                                                   data-diagnosticid="{{$row->id}}" /*envoyer l id
                                                data-original-title="" title="">
        ...
        <script>
            $(document).ready(function () {
                $('#exampleModal').on('show.bs.modal', function (event) {
                    ...
                    var id = button.data('diagnosticid');
                    //*********Modal Delete*********///////////
                    $('#hed').attr({
                        class: "modal-header alert alert-danger"
                    });
                    var title_modal = 'Supprimer diagnostic';
                    var form = $('<form>').attr({ /*baliser form on l ajoute des attributs */
                        action: '{{url('diagnostic/destroy')}}' + '/' + id + '',
                        method: "get",
                        id: "roleForm",
                        name: "form"
                    });
