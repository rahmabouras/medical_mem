@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'ordonnance', 'title' => __('ordonnance')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            ...
                            <p class="card-category"> Les Ordonnances </p>
                        </div>
                                    ...
                                    @foreach ($ordonnance as $row )
                                        <tr>
                                         ...
                                         . $row->listConsultation->DossierPatient->prenom_patient}} </td>
                                            ...
                                            <td>{{$row->nombre_de_fois}}</td>
                                            <td>{{$row->remarque}}</td>

                                            <td class="td-actions text-right">
                                                <a rel="tooltip" class="btn btn-success btn-link"
                                                   href="{{route('ordonnance.edit',$row->id)}}"
                                                   data-original-title="" title="">
                                                    <i class="material-icons">edit</i>
                                                    <div class="ripple-container"></div>
                                                </a>
                                            <!-- <form action="{{route('ordonnance.destroy',$row->id)}}" method="post">
                                          @method('DELETE')
                                            @csrf
        ...
        <script>
            $(document).ready(function () {
                $('#exampleModal').on('show.bs.modal', function (event) {
                    ...
                    var id = button.data('ordonnanceid');
                    //*********Modal Delete*********///////////
                    $('#hed').attr({
                        class: "modal-header alert alert-danger"
                    });
                    var title_modal = 'Supprimer une ordonnance';
                    var form = $('<form>').attr({ /*baliser form on l ajoute des attributs */
                        action: '{{url('ordonnance/destroy')}}' + '/' + id + '',
                        method: "get",
                        id: "roleForm",
                        name: "form"
                    });
