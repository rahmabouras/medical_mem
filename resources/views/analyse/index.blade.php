@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'analyse', 'title' => __('analyse')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            ...
                            <p class="card-category"> Voici la liste des Analyses </p>
                        </div>
                                    ...
                                    @foreach ($analyse as $row )
                                        <tr>
                                         ...
                                         . $row->listConsultation->DossierPatient->prenom_patient}}</td>
                                            ...
                                            <td>{{$row->description_analyse}}</td>
                                            <td>{{$row->resultat_analyse}}</td>

                                            <td class="td-actions text-right">
                                                <a rel="tooltip" class="btn btn-success btn-link"
                                                   href="{{route('analyse.edit',$row->id)}}"
                                                   data-original-title="" title="">
                                                    <i class="material-icons">edit</i>
                                                    <div class="ripple-container"></div>
                                                </a>
                                            <!-- <form action="{{route('analyse.destroy',$row->id)}}" method="post">
                                          @method('DELETE')
                                            @csrf
                                                ...
                                                <a data-type="delete" rel="tooltip" class="btn btn-success btn-link"
                                                   ...
                                                   data-analyseid="{{$row->id}}" /*envoyer l id
                                                data-original-title="" title="">
