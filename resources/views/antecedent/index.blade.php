@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'antecedent', 'title' => __('antecedent')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title ">Antecédent </h4>
                            <p class="card-category"> Voici les antecedents </p>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12 text-right">
                                    <a href="{{route('antecedent.create')}}">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                    @foreach ($antec as $row )
                                        <tr>
                                            <td> {{$row->DossierPatient->nom_patient.' '.$row->DossierPatient->prenom_patient}}
                                            <td>{{$row->type_antecedent}}</td>
                                            <td>{{$row->description_antecedent}}</td>

                                            <td class="td-actions text-right">
                                                <a rel="tooltip" class="btn btn-success btn-link"
                                                   href="{{route('antecedent.edit',$row->id)}}"
                                                   data-original-title="" title="">
                                                    <i class="material-icons">edit</i>
                                                    <div class="ripple-container"></div>
                                                </a>
                                            <!-- <form action="{{route('antecedent.destroy',$row->id)}}" method="post">
                                          @method('DELETE')
                                            @csrf
                                                ...
                                                <a data-type="delete" rel="tooltip" class="btn btn-success btn-link"
                                                   ...
                                                   data-antecedentid="{{$row->id}}" /*envoyer l id
                                                data-original-title="" title="">
        ...
        <script>
            $(document).ready(function () {
                $('#exampleModal').on('show.bs.modal', function (event) {
                    ...
                    var id = button.data('antecedentid');
                    //*********Modal Delete*********///////////
                    $('#hed').attr({
                        class: "modal-header alert alert-danger"
                    });
                    var title_modal = 'Supprimer un antecedent';
                    var form = $('<form>').attr({ /*baliser form on l ajoute des attributs */
                        action: '{{url('antecedent/destroy')}}' + '/' + id + '',
                        method: "get",
                        id: "roleForm",
                        name: "form"
                    });
